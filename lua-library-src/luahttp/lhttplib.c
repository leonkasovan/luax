...
static int hl_post_file (lua_State *L) {
	CURLcode rc;
	const char *url = luaL_checkstring(L, 1);// [1] parameter : http://www.myupload.com
	const char *listfile = luaL_checkstring(L, 2);	// [2] parameter : photo=C:\\My Document\\1.jpg (support one file only)
	const char *listfield = luaL_optstring(L, 3, "");	// optional : desc=My Awesome photos&age=29
	struct curl_httppost *post=NULL;
	struct curl_httppost *last=NULL;
	bstring fieldname, fieldvalue;
	const char *p;
	...
	fieldname = vstr_new_n(64,"");
	fieldvalue = vstr_new_n(MAX_PATH,"");
	
	p = listfile;
	// file: copy field name
	while(*p != '\0' && *p!='='){
		vstr_add_chr(fieldname, *p);
		p++;
	}
	if (fieldname->slen == 0){
		lua_pushinteger(L, CURLE_BAD_FUNCTION_ARGUMENT);
	...
	// file: copy field value
	while(*p != '\0'){
		vstr_add_chr(fieldvalue, *p);
		p++;
	}
	if (fieldvalue->slen == 0){
		lua_pushinteger(L, CURLE_BAD_FUNCTION_ARGUMENT);
		return 1;
	}
	curl_formadd(&post, &last, CURLFORM_COPYNAME, fieldname->data,CURLFORM_FILE, fieldvalue->data, CURLFORM_END);
	
	// other field
	p = listfield;
	while (*p){
		vstr_reset(fieldname);
		vstr_reset(fieldvalue);
		// other field: copy field name
		while(*p && *p!='='){
			vstr_add_chr(fieldname, *p);
			p++;
		}
		if (*p == '=') p++;
		// other field: copy field value
		while(*p && *p!='&'){
			vstr_add_chr(fieldvalue, *p);
			p++;
		}
		if (*p == '&') p++;
		curl_formadd(&post, &last, CURLFORM_COPYNAME, fieldname->data,CURLFORM_COPYCONTENTS, fieldvalue->data, CURLFORM_END);
	}

	vstr_free(fieldname);
	vstr_free(fieldvalue);
