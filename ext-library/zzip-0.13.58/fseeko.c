...
zzip_entry_findfirst(FILE * disk)
{
    ...
    if (fseeko(disk, 0, SEEK_END) == -1)
        return 0;
    ___ size_t disksize = ftello(disk);
    if (disksize < sizeof(struct zzip_disk_trailer))
        return 0;
    /* we read out chunks of 8 KiB in the hope to match disk granularity */
    ___ size_t pagesize = PAGESIZE; /* getpagesize() */
	...
	assert(pagesize / 2 > (zzip_off64_t) sizeof(struct zzip_disk_trailer));
    ...
    ___ size_t mapoffs = disksize & ~(pagesize - 1);
    ___ size_t mapsize = disksize - mapoffs;
    ...
    while (1)
    {
        ...
        for (; p >= buffer; p--)
        {
            size_t root;    /* (struct zzip_disk_entry*) */
            if (zzip_disk_trailer_check_magic(p))
            {
                ...
                if (root > disksize - (long) sizeof(struct zzip_disk_trailer))
                {
                    ...
                    size_t rootsize = zzip_disk_trailer_rootsize(trailer);
                    if (rootsize > mapoffs)
                        continue;
